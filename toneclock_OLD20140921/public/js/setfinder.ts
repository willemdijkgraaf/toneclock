...
export module ToneClock {
    export class SetFinder {
        sNAME = "Tone Clock Set Finder";
        sVERSION = "0.1";

        primedata = new Array(352);

        card = 0;  // cardinality = number of boxes checked
        curimg = "i01";
        adj:number = 0;
...
// This is done to make it easier to compute rotations (when looking for the normal form)
        ...
        pcOriginal = new Array(24);   // original checked numbers
        pcInversion = new Array(24);  // inversions of the checked numbers (12-x)
        ...
        pcOrigPPrime = new Array(12);
        pcInvPPrime = new Array(12);
        pcPPrime = new Array(12);
        ...
        public Initialize()        // reset properties
        {
            this.pcPrime = "";
            this.pcForte = "";
            this.pcIVec = "";
        ...
        public FindSet(pitchClassArray) { // pitchClassArray : is an array of 12 elements of type boolean. The cardinality represents the pitch class. True will include that pitch class
            // Compute Cardinality and create 1st half of the pcOriginal array
            this.card = 0;
            for (var i = 0; i < 12; i++) {
                if (pitchClassArray[i]) {
                    this.pcOriginal[this.card] = i;
                    this.card++;
                }
            }
            ...
            for (i = 0; i < this.card; i++)
                this.pcOriginal[this.card + i] = this.pcOriginal[i] + 12;

            // create pcInversion array
            for (i = 0; i < this.card; i++) {
                this.pcInversion[i] = 12 - this.pcOriginal[this.card - i - 1];
                this.pcInversion[this.card + i] = this.pcInversion[i] + 12;
            }
        ...
        private ComputePrime() {
            ...
            if (this.card == 0) return;

            // isForte = document.PCSForm.alg[0].checked;
            ...
            if (sc1 != -1 && sc2 != -1 && sc3 != -1) {
                this.pcForte = sTmp.substring(0, sc1);
                this.pcIVec = sTmp.substring(sc1 + 1, sc2);
                this.curimg = sTmp.substring(sc2 + 1, sc3);
                if (sTmp.substring(sc3 + 1, 99999) != " ")
                    this.pcInv = sTmp.substring(sc3 + 1, 99999);
                else
        ...
        private ComputePartialPrime(pc, pcout) {
            ...
            for (var i = 1; i < this.card; i++) {
                ...
                if ((pc[i + this.card - 1] - pc[i]) == (pc[best + this.card - 1] - pc[best])) {
                    for (var j = 1; j < (this.card - 1); j++) {
                        if (this.isForte)  adj = j;
                        else         adj = this.card - j - 1;
        ...
        private ComparePartialPrimes() {
            var bestPrime = 0;  // Assume pcOrigPrime is best until we know otherwise

            for (var i = 0; i < this.card; i++) {
                if (this.isForte)  this.adj = i;
                else         this.adj = this.card - i - 1;
            ...
            if (bestPrime == 0) {
                for (i = 0; i < this.card; i++)  this.pcPPrime[i] = this.pcOrigPPrime[i];
            }
            else {
                for (i = 0; i < this.card; i++)  this.pcPPrime[i] = this.pcInvPPrime[i];
            }
        }
    }
}